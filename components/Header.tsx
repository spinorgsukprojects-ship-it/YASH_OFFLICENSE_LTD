...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-emerald-950/10 bg-cream/95 backdrop-blur">
      <div className="bg-emerald-950 py-2 text-sm font-bold text-white">
        <div className="container flex flex-wrap items-center justify-center gap-x-6 gap-y-2 md:justify-between">
          <span className="inline-flex items-center gap-2"><Clock size={16} /> Open late for drinks, snacks and essentials</span>
      ...
      <div className="container flex min-h-20 items-center justify-between gap-4">
        <Link href="/" className="leading-tight" onClick={() => setOpen(false)}>
          ...
          <span className="text-xs font-extrabold uppercase tracking-[0.18em] text-burgundy">Food, wine & daily essentials</span>
        </Link>
        <nav className="hidden items-center gap-1 lg:flex" aria-label="Main navigation">
          {nav.map(([label, href]) => (
            <Link
              key={href}
              href={href}
              className={`rounded-md px-3 py-2 text-sm font-extrabold transition hover:bg-white ${pathname === href ? "text-burgundy" : "text-emerald-950"}`}
            >
              {label}
            </Link>
          ))}
